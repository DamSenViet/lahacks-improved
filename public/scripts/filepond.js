	...
	var styles = function styles(_ref) {
		...
		return {
			write: function write() {
				...
				Object.assign.apply(
					Object, [currentProps].concat(toConsumableArray(viewProps))
				);
...
*/
	var createProcessorFunction = function createProcessorFunction() {
		...
		if (typeof action === 'function') {
			return function() {
				for (
					var _len = arguments.length, params = Array(_len), _key = 0; _key < _len; _key++
				) {
 ...
 */
		...
		return {
			...
			ADD_ITEM: function ADD_ITEM(_ref3) {
				...
				item.on('load-file-error', function(error) {
					dispatch('DID_THROW_ITEM_INVALID', _extends({}, error, {
						id: id
					}));
				});
	...
	var file = createView({
		...
		didCreateView: function didCreateView(root) {
			applyFilters('CREATE_VIEW', _extends({}, root, {
				view: root
			}));
		},
		name: 'file'
	...
	var create$1 = function create(_ref) {
		...
		root.ref.label = root.appendChildView(
			root.createChildView(dropLabel, _extends({}, props, {
				translateY: null
			}))
		);
	...
	var createApp$1 = function createApp() {
		...
		var processFiles = function processFiles() {
			for (
				var _len2 = arguments.length, args = Array(_len2), _key2 = 0; _key2 < _len2; _key2++
			) {
	...
	var registerPlugin = function registerPlugin() {
		for (
			var _len = arguments.length, plugins = Array(_len), _key = 0; _key < _len; _key++
		) {
